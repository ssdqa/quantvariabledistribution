...
After following the instructions to reproduce the analysis, you will also need to change the `output_function` column to tell the `qvd_output` function which check you executed. Reference the table below for the labels that are associated with each check:
...
dplyr::tibble('Check Type' = c('Multi Site, Exploratory, Cross-Sectional',
              ...
              'output_function' = c('qvd_ms_exp_cs',
                                    'qvd_ms_exp_la',
                                    'qvd_ms_anom_cs',
                                    'qvd_ms_anom_la')) %>%
  knitr::kable()
...
For a cross-sectional analysis, the `compute_kl_divergence` function in this package should be executed against your results. This will compute the Kullback-Leibler divergence of each site's value distribution to the overall value distribution.

The `kl_log_base` value can be selected by the user out of the following options: `log`, `log2`, or `log10`. The standard default is `log2`.
...
For a longitudinal analysis, the `qvd_euclidean` function in this package should be executed against your results. This will compute the Euclidean distance from the site's time series to the overall time series.

The `euclidean_stat` value can be either `mean` or `median`, as desired by the user.
